...
static void SPI_TJA1145_cfg(spi_num_t spi_num, spi_baudrate_t baud_rate, spi_state_t mtfe)
{
    ...
    if (spi_num == 1){
        ...
        TJADBG("SPI_1_CLK_MSCR_NB = %08x\n",SIUL2.MSCR[SPI_1_CLK_MSCR_NB].R);
        TJADBG("SPI_1_SOUT_MSCR_NB = %08x\n",SIUL2.MSCR[SPI_1_SOUT_MSCR_NB].R);
        TJADBG("SPI_1_SIN_MSCR_NB = %08x\n",SIUL2.MSCR[SPI_1_SIN_MSCR_NB].R);
        TJADBG("SPI_1_SIN_IMCR_NB = %08x\n",SIUL2.MSCR[SPI_1_SIN_IMCR_NB].R);
        TJADBG("SPI_1_CS2_MSCR_NB = %08x\n",SIUL2.MSCR[SPI_1_CS2_MSCR_NB].R);
    } else {
    ...
    TJADBG("SPI->MODE.CTAR[2].R = %08x\n",SPI_TJA1145->MODE.CTAR[2].R);
...
HAL_SPI_StateTypeDef HAL_SPI_TransmitReceive(uint8_t *pTxData,uint8_t *pRxData,uint16_t Size,uint32_t Timeout)
{
    ...
    if((pTxData == NULL) || (pRxData == NULL) || (Size == 0))
    {
      ...
      TJADBG("SPI buffer or size error,size = %d\n",Size);
      goto error;
    }
    ...
    for(uint8_t cnt=0;cnt<Size;cnt++)
    {
       ...
       TJADBG("SPI tx ====================\n",cnt);
       TJADBG("SPI tx size = %d \n",cnt);
       TJADBG("SPI tx data = %02x , complete tx = %08x\n",*(pTxData + cnt) , (tx_action | (*(pTxData + cnt))));
       //TJADBG("Before SPI RFDF = %01x , SPI TCF = %01x , SPI timeout = %d\n",SPI->SR.B.RFDF ,SPI->SR.B.TCF ,timeout);
        while((!SPI_TJA1145->SR.B.RFDF || !SPI_TJA1145->SR.B.TCF) && timeout > 0)
        {
        	timeout--;
        }
        //TJADBG("After SPI RFDF = %01x , SPI TCF = %01x , SPI timeout = %d\n",SPI->SR.B.RFDF ,SPI->SR.B.TCF ,timeout);
        receive_data = SPI_TJA1145->POPR.R;
        *(pRxData + cnt) = (uint8_t)(receive_data & 0x000000FF);
        TJADBG("SPI rx data = %02x , complete rx = %08x\n",*(pRxData + cnt) , receive_data);
        SPI_TJA1145->SR.B.TCF = 1;
        SPI_TJA1145->SR.B.RFDF = 1;
        if(0 == timeout)
		{
			TJADBG("SPI transmit timeout (%d)\n" ,cnt);
			goto error;
		}
        timeout = Timeout;
    ...
    error:
		SIUL2.DIRER0.B.EIRE0 = 0x01;
		TJADBG("SPI Fail and return error code\n");
	return errorcode;
}
