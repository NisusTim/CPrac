///////////////////////////////////////////////////////////////////////////////////////////////////
// ��蕭 NXP B.V. 2013. All rights reserved.
// Disclaimer
...
* \version	1.0
* \date     2013/06/05
* \return	<b>StdRet_t</b>  Function executed successfully or not
*                               possible values: RET_OK, RET_NOT_OK
*/  
StdRet_t StartupOperation(void){
	StdRet_t ret = RET_OK;
	Byte result;          		
	...
	if (DeviceType == NXP_TJA1145FD){           	  
		...
		if (result == 0) {              
			...
			// Check FSMS setting
			if (CheckFsmStatus() == RET_NOT_OK){
				ret = RET_NOT_OK;
			}

			// Event Handling
			if (EventHandling() == RET_NOT_OK){
				ret = RET_NOT_OK;
			}

			// Change to Normal Operation
			if (ChangeToNormalOperation() == RET_NOT_OK){
				ret = RET_NOT_OK;
			}
		}  else {
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t ChangeToStandbyOperation(){
	StdRet_t ret = RET_OK;
	...
	if(mc_reg != TJA1145FD_MC_STANDBY_MODE) {
		...
		if ((trcvEvEn.TJA1145FD_CWE == TJA1145FD_CWE_CAN_WAKEUP_DETECTION_DISABLE) && 
			...
			if (TJA1145_SetPNActivationState((CanTrcv_PNActivationType) TJA1145FD_CPNC_STANDBY) != RET_OK){
				ret = RET_NOT_OK;
			}
			(void) TJA1145FD_setLockControl(TJA1145FD_LK6C_SPI_WRITE_ACCESS_DISABLED_0X68_TO_0X6F, TJA1145FD_LK5C_SPI_WRITE_ACCESS_DISABLED_0X50_TO_0X5F, TJA1145FD_LK4C_SPI_WRITE_ACCESS_ENABLED_0X40_TO_0X4F, TJA1145FD_LK3C_SPI_WRITE_ACCESS_DISABLED_0X30_TO_0X3F, TJA1145FD_LK2C_SPI_WRITE_ACCESS_DISABLED_0X20_TO_0X2F, TJA1145FD_LK1C_SPI_WRITE_ACCESS_DISABLED_0X10_TO_0X1F, TJA1145FD_LK0C_SPI_WRITE_ACCESS_DISABLED_0X06_TO_0X09);
			...
			if((pendingEvent.TJA1145FD_WPE == TJA1145FD_WPE_NO_WAKE_PIN_EVENT) &&
				...
				(pendingEvent.TJA1145FD_SYSE == TJA1145FD_SYSE_NO_SYSTEM_EVENT)){                             
					...
					if ((ret == RET_OK) && (TJA1145FD_CPNC_STANDBY == TJA1145FD_CPNC_CAN_SELECTIVE_WAKEUP_ENABLE)){        
						// dedicated request to clear the WUF flag
						if (TJA1145_ClearTrcvWufFlag() == RET_NOT_OK){
							ret = RET_NOT_OK;
						}	
						// Disable CAN PE; wake-up disabled
						(void) CANStopMode();
						// Enter TJA1145 Standby Mode
						if (TJA1145_SetOpMode(CANTRCV_TRCVMODE_STANDBY) == RET_NOT_OK){
							ret = RET_NOT_OK;
						}	
						// Disable CAN PE; wake-up enabled
						(void) CANSleepMode();	
						// Check if WUF has occured meanwhile; If yes, wake up ECU again
						if (TJA1145_CheckWakeFlag() == RET_NOT_OK){
							ret = RET_NOT_OK;
						}
					}    	  
					// If partial networking is not activated in TJA1145 
					else if ((ret == RET_OK) && (TJA1145FD_CPNC_STANDBY == TJA1145FD_CPNC_CAN_SELECTIVE_WAKEUP_DISABLE)) {
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t ChangeToSleepOperation(){
	StdRet_t ret = RET_OK;
	...
	if(mc_reg != TJA1145FD_MC_SLEEP_MODE) {
		...
		if ((trcvEvEn.TJA1145FD_CWE == TJA1145FD_CWE_CAN_WAKEUP_DETECTION_DISABLE) && 
			...
			if (TJA1145_SetPNActivationState((CanTrcv_PNActivationType) TJA1145FD_CPNC_SLEEP) != RET_OK){
				ret = RET_NOT_OK;
			}
			(void) TJA1145FD_setLockControl(TJA1145FD_LK6C_SPI_WRITE_ACCESS_DISABLED_0X68_TO_0X6F, TJA1145FD_LK5C_SPI_WRITE_ACCESS_DISABLED_0X50_TO_0X5F, TJA1145FD_LK4C_SPI_WRITE_ACCESS_ENABLED_0X40_TO_0X4F, TJA1145FD_LK3C_SPI_WRITE_ACCESS_DISABLED_0X30_TO_0X3F, TJA1145FD_LK2C_SPI_WRITE_ACCESS_DISABLED_0X20_TO_0X2F, TJA1145FD_LK1C_SPI_WRITE_ACCESS_DISABLED_0X10_TO_0X1F, TJA1145FD_LK0C_SPI_WRITE_ACCESS_DISABLED_0X06_TO_0X09);
			...
			if (EventHandling() == RET_NOT_OK){
				//ret = RET_NOT_OK;
				TJADBG("TJA1145 EventHandling RET_NOT_OK\n");
			}
			// If no interrupt is pending, enter TJA1145 Standby Mode
			(void) TJA1145FD_getGlobalEventStatus(&pendingEvent.TJA1145FD_WPE, &pendingEvent.TJA1145FD_TRXE, &pendingEvent.TJA1145FD_SYSE);
			...
			if((pendingEvent.TJA1145FD_WPE == TJA1145FD_WPE_NO_WAKE_PIN_EVENT) &&
				...
				(pendingEvent.TJA1145FD_SYSE == TJA1145FD_SYSE_NO_SYSTEM_EVENT)){                             
					...
					// If PN has been activated successfully, follow defined shutdown flow for shutdown of CAN 
					//  transceiver and CAN controller (source: Autosar R3.2 and higher)
					if ( ret == RET_OK && (TJA1145FD_CPNC_SLEEP == TJA1145FD_CPNC_CAN_SELECTIVE_WAKEUP_ENABLE)){
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t ChangeToNormalOperation(){
	StdRet_t ret = RET_OK; 
...
StdRet_t EventHandling() {
	...
	if (pendingEv.TJA1145FD_SYSE == TJA1145FD_SYSE_SYSTEM_EVENT_PENDING) {
		...
		if(sysEv.TJA1145FD_PO == TJA1145FD_PO_OFF_MODE_LEFT) {
			TJADBG("TJA1145 System PO = 1\n");
			if (TJA1145_PO_ServiceRoutine() == RET_NOT_OK){
				ret = RET_NOT_OK;
			}
		...
		if(sysEv.TJA1145FD_OTW == TJA1145FD_OTW_TEMPERATURE_EXCEEDS_WARNING_LEVEL) {
			TJADBG("TJA1145 System OTW = 1\n");
			if (TJA1145_OTW_ServiceRoutine() == RET_NOT_OK){
				ret = RET_NOT_OK;
			}	
		}
		// SPIF Event pending
		if(sysEv.TJA1145FD_SPIF == TJA1145FD_SPIF_SPI_FAILURE_DETECTED) {
			TJADBG("TJA1145 System SPIF = 1\n");
			if (TJA1145_SPIF_ServiceRoutine() == RET_NOT_OK){
				ret = RET_NOT_OK;
			}
		}		
	}
	...
	if (pendingEv.TJA1145FD_TRXE == TJA1145FD_TRXE_TRANSCEIVER_EVENT_PENDING) {
		...
		if(trcvEv.TJA1145FD_PNFDE == TJA1145FD_PNFDE_PARTIAL_NETWORK_FRAME_ERROR_DETECTED) {
			TJADBG("TJA1145 Transceiver PNFDE = 1\n");
			if (TJA1145_PNFDE_ServiceRoutine() == RET_NOT_OK){
				ret = RET_NOT_OK;
			}
		...
		if(trcvEv.TJA1145FD_CW == TJA1145FD_CW_CAN_WAKEUP_EVENT) {
			TJADBG("TJA1145 Transceiver CW = 1\n");
			if (TJA1145_CW_ServiceRoutine() == RET_NOT_OK){
				ret = RET_NOT_OK;
			}
		}
		// CF Event pending
		if(trcvEv.TJA1145FD_CF == TJA1145FD_CF_CAN_FAILURE_DETECTED) {
			TJADBG("TJA1145 Transceiver CF = 1\n");
			if (TJA1145_CF_ServiceRoutine() == RET_NOT_OK){
				ret = RET_NOT_OK;
			}
		}
		// CBS Event pending
		if(trcvEv.TJA1145FD_CBS == TJA1145FD_CBS_NO_CAN_BUS_ACTIVITY) {
			TJADBG("TJA1145 Transceiver CBS = 1\n");
			if (TJA1145_CBS_ServiceRoutine() == RET_NOT_OK){
				ret = RET_NOT_OK;
			}
		}
	}
	...
	if (pendingEv.TJA1145FD_WPE == TJA1145FD_WPE_WAKE_PIN_EVENT_PENDING) {
		...
		if(wakeEv.TJA1145FD_WPR == TJA1145FD_WPR_WAKEPIN_RISING_EDGE_DETECTED) {
			TJADBG("TJA1145 Wake Pin WPR = 1\n");
			if (TJA1145_WPR_ServiceRoutine() == RET_NOT_OK){
				ret = RET_NOT_OK;
			}
			// save wake-up reason
		...
		if(wakeEv.TJA1145FD_WPF == TJA1145FD_WPF_WAKEPIN_FALLING_EDGE_DETECTED) {
			TJADBG("TJA1145 Wake Pin WPF = 1\n");
			if (TJA1145_WPF_ServiceRoutine() == RET_NOT_OK){
				ret = RET_NOT_OK;
			}
			// save wake-up reason
			WuReason = CANTRCV_WU_BY_PIN;
...
*                               possible values: RET_OK, RET_NOT_OK	
*/
StdRet_t TJA1145_SetPnIdentifier(Word id, TJA1145FD_Identifier_Format_t idFormat){
	StdRet_t ret = RET_OK;
...
*                               possible values: RET_OK, RET_NOT_OK	
*/
StdRet_t TJA1145_SetPnIdMask(Word mask, TJA1145FD_Identifier_Format_t idFormat){
	StdRet_t ret = RET_OK;
...
StdRet_t ConfigurePartialNetworking(){
	StdRet_t ret = RET_OK;
	...
	if (TJA1145FD_setDataRate(TJA1145FD_CAN_DATA_RATE) != NXP_UJA11XX_SUCCESS){
		ret = RET_NOT_OK;
	}
	// configure CAN ID registers
	if (TJA1145_SetPnIdentifier(UJA116FDVX_CAN_ID, TJA1145FD_CAN_FRAME_CONTROL.TJA1145FD_IDE) == RET_NOT_OK){
		ret = RET_NOT_OK;
	}
	// configure CAN ID mask registers
	if (TJA1145_SetPnIdMask(TJA1145FD_CAN_ID_MASK, TJA1145FD_CAN_FRAME_CONTROL.TJA1145FD_IDE) == RET_NOT_OK){
		ret = RET_NOT_OK;
	}
	...
	if (ret == RET_OK){
		if (TJA1145FD_setCANControl(cc.TJA1145FD_CFDC, TJA1145FD_PNCOK_PARTIAL_NETWORK_CONFIGURATION_SUCCESSFUL, TJA1145FD_CPNC_CAN_SELECTIVE_WAKEUP_ENABLE, cc.TJA1145FD_CMC) != NXP_UJA11XX_SUCCESS) {
			ret = RET_NOT_OK;
		}
	}
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t TJA1145_SPIF_ServiceRoutine() {
	StdRet_t ret = RET_OK;
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t TJA1145_OTW_ServiceRoutine(){
	StdRet_t ret = RET_OK;
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t TJA1145_PO_ServiceRoutine(){
	StdRet_t ret = RET_OK;
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t TJA1145_PNFDE_ServiceRoutine(){
	StdRet_t ret = RET_OK;
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t TJA1145_CW_ServiceRoutine(){
	StdRet_t ret = RET_OK;
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t TJA1145_CF_ServiceRoutine(){
	StdRet_t ret = RET_OK;
	TJA1145FD_Transceiver_Status_Reg_t trcvStatus;
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t TJA1145_CBS_ServiceRoutine() {
	StdRet_t ret = RET_OK;  
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t TJA1145_WPR_ServiceRoutine(){
	StdRet_t ret = RET_OK;
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t TJA1145_WPF_ServiceRoutine(){
	StdRet_t ret = RET_OK;
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t TJA1145_GetWuReason(CanTrcv_TrcvWakeupReasonType* pReason) {
	StdRet_t ret = RET_OK;
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t TJA1145_ClearTrcvWufFlag(){
	StdRet_t ret = RET_OK;
	TJA1145FD_CAN_Control_Reg_t canCtrl;
	TJA1145FD_Transceiver_Event_Status_Reg_t trcvEv;     	
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t TJA1145_ReadTrcvTimeoutFlag(TJA1145FD_CAN_Bus_Active_t* pCbs){
	StdRet_t ret = RET_OK;    
	TJA1145FD_Transceiver_Event_Status_Reg_t trcvEv;
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t TJA1145_ClearTrcvTimeoutFlag() {
	StdRet_t ret = RET_OK;
	TJA1145FD_Transceiver_Event_Status_Reg_t trcvEv;
	...
	if (TJA1145FD_setTransceiverEventStatus(trcvEv.TJA1145FD_PNFDE, TJA1145FD_CBS_NO_CAN_BUS_ACTIVITY, trcvEv.TJA1145FD_CF, trcvEv.TJA1145FD_CW) != NXP_UJA11XX_SUCCESS){
		ret = RET_NOT_OK;
	}
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t TJA1145_ReadTrcvSilenceFlag(TJA1145FD_CAN_Bus_Status_t* pCbss) {
	StdRet_t ret = RET_OK;      
...
*	ECU shall be woken-up again. This is signalled by return value RET_NOT_OK.
*	- In case a pin or any other wake-up occurs in between change of the MCU from Normal to Standby Operation,
* in this example software the ECU shall also be woken up. This is signalled by return value RET_NOT_OK.
*	
* \author   jh
* \version  1.0
* \date     2013/06/05
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t TJA1145_CheckWakeFlag(){
	StdRet_t ret = RET_OK;
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t TJA1145_SetPNActivationState(CanTrcv_PNActivationType actState) {
	StdRet_t ret = RET_OK;
	TJA1145FD_CAN_Control_Reg_t canCtrl;  	
...
*                               possible values: RET_OK, RET_NOT_OK
*/
StdRet_t TJA1145_GetOpMode(CanTrcv_TrcvModeType* pMode){
	StdRet_t ret = RET_OK;
...
*/
StdRet_t TJA1145_SetOpMode(CanTrcv_TrcvModeType mode){
	StdRet_t ret = RET_OK;
...
void TJA1145_init()
{
	  TJADBG("TJA1145 init start\n");
	  StartupOperation();
	  ChangeToNormalOperation();
	  ConfigurePartialNetworking();
}
