...
#define FS6500_PROGRAM1     1
#define FS6500_PROGRAM2     0

#define	DSPI_NB	2					///defines number of DSPI module
#define	DSPI_CS	3			///defines Chip Select
...
static void PIT_wait_micsec(int msecs) //yilin for test
{
	for(int i = 0; i < 100*msecs; )
		i++;
}
 ...
 *		operation for the LDT. Resolutions is 488?�c
 *   @par Include
 *		FS65xx.h
 *   @par Description
...
uint32_t FS65_ConfAfterRunValue(uint32_t value){
    ...
    errorCode =  FS65_SendCmdW(cmd1.R);
    ...
    errorCode = FS65_SendCmdW(cmd2.R);
    return errorCode;
...
uint32_t FS65_ConfWakeUpValue(uint32_t value){
    ...
    cmd1.B.B23_16 = (value & 0x000000FF);
    ...
    cmd3.B.B7_0 = ((value & 0x00FF0000)>>16);

    errorCode = FS65_SendCmdW(cmd3.R);
    return errorCode;
...
uint32_t FS65_SendCmdR(uint32_t cmd){
    ...
    stockPriority = INTC.CPR[0].B.PRI; 			//save current priority
    INTC.CPR[0].B.PRI = INT_CEIL_PRIORITY;		//block DSPI resource
    ...
    if(SPIstruct.statusPwSBC.B.SPI_G == 1){
    	if(SPIstruct.response == 0xFFFF){
    		...
    		if((INTstruct.DIAG_SPI.B.bit0 & INTstruct.DIAG_SPI.B.bit2) == 0){
    			INTC.CPR[0].B.PRI = stockPriority;					//release DSPI resource
