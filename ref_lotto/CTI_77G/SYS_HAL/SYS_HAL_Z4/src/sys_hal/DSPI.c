...
// Endo of Queue flag
#define SPI_EOQ_FLAG             0x08000000
// Keep CS asserted between bytes
#define SPI_CONT_FLAG            0x80000000
// Chip select flag (CS0)
#define SPI_CS0_FLAG             0x00010000
...
void DSPI_Init(uint8_t DspiNumber,uint8_t mode, uint32_t sysClk, uint32_t baudRate, uint32_t intMask)		
{
	...
	SIUL2.MSCR[SPI_2_CLK_MSCR_NB].R = 0x12840000 | SPI_2_CLK_MSCR_SSS;		// PA11/F15: configure SPI2-SCK  -> PA11 on MPC57xx Motherboard
	SIUL2.MSCR[SPI_2_SOUT_MSCR_NB].R = 0x12840000 | SPI_2_SOUT_MSCR_SSS;	// PA12/B16: configure SPI2-SOUT -> PA12 on MPC57xx Motherboard
	SIUL2.MSCR[SPI_2_SIN_MSCR_NB].R = 0x008C0000;							// PA13/D17: configure SPI2-SIN  -> PA13 on MPC57XX Motherboard
	SIUL2.IMCR[SPI_2_SIN_IMCR_NB].R  = 0x00000000 | SPI_2_SIN_IMCR_SSS;		//
	SIUL2.MSCR[SPI_2_CS3_MSCR_NB].R = 0x12840000 | SPI_2_CS3_MSCR_SSS;		// PA44/C16: configure SPI2-CS0  -> PA10 on MPC57xx Motherboard
	SIUL2.MSCR[SPI_2_CS0_MSCR_NB].R = 0x12840000;							// PA10/C16: configure SPI2-CS0  -> PA10 on MPC57xx Motherboard

	// check if fp is 80 MHz
	if(CGM.AC12_DC0.R != 0x80020000)
    ...
    {											//choose base DSPI address
        ...
        default: p_DSPI = &SPI_2; break;
    }

	//p_DSPI = &SPI_2;
    p_DSPI->MCR.R = 0x80FF0E01;
    p_DSPI->MCR.B.MSTR = 1;		// Set DSPIx in mode Slave-0 or Master-1
    p_DSPI->MCR.B.CONT_SCKE = 0;// No continuous clock
	p_DSPI->MCR.B.DCONF = 0;    // SPI

	p_DSPI->MCR.B.PES = 0;      // Don't stop on parity error
    p_DSPI->MCR.B.PCSIS=0xFF;	// CS0-5 active Low
    ...
    p_DSPI->MCR.B.FRZ = 0;
    p_DSPI->MCR.B.CLR_TXF = 1;     // Clear TX FIFO
    p_DSPI->MCR.B.CLR_RXF = 1;     // Clear RX FIFO
    p_DSPI->MCR.B.SMPL_PT = 2;     // 2 fp clock cycles between SCK and SIN (MTFE only!)    p_DSPI->MCR.B.PES = 0;           // Don't stop on parity error
    p_DSPI->MCR.B.HALT = 1;     // Keep HALT=1 (stop transfers)
    p_DSPI->MCR.B.MDIS = 0;     // Enable module

    p_DSPI->MODE.CTAR[3].B.DBR=0; 	// double baud rate
    p_DSPI->MODE.CTAR[3].B.FMSZ=15; // Frame size (value+1)
    p_DSPI->MODE.CTAR[3].B.CPOL=0; 	// Polarity CPOL=0=>sck is valid @1
    p_DSPI->MODE.CTAR[3].B.CPHA=1; 	// Phase CPHA=0=>data captured on leading edge
    p_DSPI->MODE.CTAR[3].B.LSBFE=0; // MSB first if 0

    if(mode == MASTER)
    {
        p_DSPI->MODE.CTAR[3].B.PBR=binaryPBR;		//Baud rate
        p_DSPI->MODE.CTAR[3].B.BR=binaryBR;
        //p_DSPI->MODE.CTAR[3].B.PBR=2;				//Baud rate
        //p_DSPI->MODE.CTAR[3].B.BR=4;
        PRINTF("binaryPBR = %x\n", binaryPBR);
        PRINTF("binaryBR = %x\n", binaryBR);
    }

    p_DSPI->MODE.CTAR[3].B.PCSSCK = PRESC_VAL1;		//tcsc
    p_DSPI->MODE.CTAR[3].B.CSSCK = SCALER3;

    p_DSPI->MODE.CTAR[3].B.PDT = PRESC_VAL3;		//tdt
    p_DSPI->MODE.CTAR[3].B.DT = SCALER4;

    p_DSPI->MODE.CTAR[3].B.PASC = PRESC_VAL1;		//tasc
    p_DSPI->MODE.CTAR[3].B.ASC = SCALER3;

    p_DSPI->SR.R = 0xFFFFFFFF;						// clear all flags
    p_DSPI->MCR.B.HALT = 0;							// start transfers
...
void DSPI_Send(uint8_t DspiNumber,uint8_t CSmask,uint16_t Word)
{
	...
	toSend = 0x30000000 | (0x1 << (CSmask+16));
    toSend = toSend & 0x30FF0000;
   ...
   p_DSPI->PUSHR.PUSHR.R = toSend;
    if(p_DSPI->MCR.B.MSTR==MASTER){
	    ...
	    if(secure_counter > DSPI_SECURE_COUNTER)
	    	PRINTF("Do SPI time out\n");
