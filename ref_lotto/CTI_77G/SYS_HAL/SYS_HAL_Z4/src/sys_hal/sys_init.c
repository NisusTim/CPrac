...
const LIBS_VER_T __attribute__((section(".library_version"))) SysHal_Version =  {
		.id =          VER_ID          (SYS_HAL_LIB_VER),
		.major =       VER_MAJOR       (3),
		.minor =       VER_MINOR       (5),
...
void z7s_up(uint32_t entry_z70, uint32_t entry_z71)
{
	...
	if(entry_z70) {
		...
		MC_ME.CADDR2.R = entry_z70 | 0x1;
	}

	/* enable core 2 in all modes */
	if(entry_z71) {
		MC_ME.CCTL3.R = 0x00FE;
		MC_ME.CADDR3.R = entry_z71 | 0x1;
	}

	if(entry_z70 || entry_z71) {
		MC_ME.MCTL.R = (mctl & 0xffff0000ul) | KEY_VALUE1;
		MC_ME.MCTL.R =  mctl; /* key value 2 always from MCTL */
	}
}

void z7s_mode(uint32_t mode, uint16_t cctl2, uint16_t cctl3, uint32_t z70rchw, uint32_t z71rchw)
{
    ...
    //MC_ME.CADDR1.R = Z4_HDR->rchw2;
    MC_ME.CADDR2.R = z70rchw | 0x1;
    MC_ME.CADDR3.R = z71rchw | 0x1;

	MC_ME.MCTL.R = (mode & 0xF0000000) | KEY_VALUE1;

	MC_ME.MCTL.R = (mode & 0xF0000000) | KEY_VALUE2; /* key value 2 always from MCTL */

    while(MC_ME.GS.B.S_MTRANS == 1) ;           /* Wait for mode entry to complete */
}

void z7s_ctrl(char** args, int count)
{
	uint32_t mode = 0x30000000, z70rchw=0, z71rchw=0;
	uint16_t cctl2 = 0x00FE, cctl3 = 0x00FE;

	if(count < 4)
		return;

	mode = strtoul(args[1], NULL, 16);
	cctl2 = strtoul(args[2], NULL, 16);
	cctl3 = strtoul(args[3], NULL, 16);

	if(count > 4)
		z70rchw = strtoul(args[4], NULL, 16);

	if(count > 5)
		z71rchw = strtoul(args[5], NULL, 16);

	z7s_mode(mode, cctl2, cctl3, z70rchw, z71rchw);
}
