...
/** \brief A alias of the struct _tag_LogicalBlockHeader.*/
typedef struct _tag_LogicalBlockHeader bl_LogicalBlockHeader_t;
...
/**
 *  \brief This is a header of a logical blocks.
 */
struct _tag_LogicalBlockHeader
{
    bl_u32_t verBT;
    bl_u32_t verAPP;
};
...
#endif
/** \brief read the fingerprint information of the logical blocks.*/
extern bl_Return_t Lbm_ReadFingerprint(bl_Buffer_t *buffer,
                                             bl_BufferSize_t *respSize);

extern bl_Return_t Lbm_ReadDownloadDataandCalCheckSum(  bl_Address_t address,
														bl_Size_t size,
														bl_u16_t *checksum);
