...
#define FLASHIF_INIT(pp)    FlashDriInit(pp)
#define FLASHIF_DEINIT(pp)  FlashDriDeinit(pp)
#define FLASHIF_WRITE(pp)   FlashDriWrite(pp)
#define FLASHIF_ERASE(pp)   FlashDriErase(pp)
#define FLASHIF_READ(pp)    Flash_Read(pp)
 ...
 *****************************************************************************/
static FlsDriverStatusType FlsDriverInitStatus = FlsDriverStatus_Init;
/*****************************************************************************
...
bl_Return_t Flsif_Init(void)
{
    ...
    if(BL_ERR_OK == ret)
    {
        FlsDriverInitStatus = FlsDriverStatus_Init;
    }
    return ret;
}
...
bl_Return_t Flsif_DeInit(void)
{
    ...
    {
        FlsDriverInitStatus = FlsDriverStatus_UInit;
    }
                                        ...
                                        bl_Size_t size)
{
    ...
    if(FlsDriverStatus_Init == FlsDriverInitStatus)
    {
#if (FLASHIF_FUN_CONVERT_ADDRESS == BL_FUN_ON)
        FLASHIF_CONVERTADDRESS(address, &phyAddr);
#else
                                        ...
                                        const bl_Buffer_t *buffer)
{
    ...
    g_ImgSecOps.decryption(address, tmpBuf, size);

    for (; i < 8u; i++)
    {
        tmpBuf[i] = FLASH_ERASE_VALUE;
    }

    param.wrdata = (bl_u64_t)((bl_u64_t)tmpBuf[7]|
                             ((bl_u64_t)tmpBuf[6]<<8)|
                             ((bl_u64_t)tmpBuf[5]<<16)|
                             ((bl_u64_t)tmpBuf[4]<<24)|
                             ((bl_u64_t)tmpBuf[3]<<32)|
                             ((bl_u64_t)tmpBuf[2]<<40)|
                             ((bl_u64_t)tmpBuf[1]<<48)|
                             ((bl_u64_t)tmpBuf[0]<<56));

    param.address = address;

    param.size = size;
    param.buffer = (bl_Buffer_t *)buffer;
#if(RTE_FUN_WATCHDOG == BL_FUN_ON)
#endif
    g_RteInfo.fs8xOps->Feed();
    param.FeedDog = g_RteInfo.wdgOps->Feed;
    param.Fs8xFeedDog = g_RteInfo.fs8xOps->Feed;
    if((param.address>= 0xF98000)&&(param.address<= 0x0117FFFF))
    {
    	param.fs8x_wd_ftime = FS8x_Get_Last_Feed_Timestamp();
        FLASHIF_WRITE(&param);
        FS8x_Set_Last_Feed_Timestamp(param.fs8x_wd_ftime);
    }
    else if((param.address>= 0x800000)&&(param.address<= 0x817FFF))
    {
        DFlashWrite(&param);
    }
    else
    {
        param.errCode = BL_ERR_NOT_OK;
    }
