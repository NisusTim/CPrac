...
#define LBM_IS_VIRTUAL(lb)          (LBM_TYPE_VIRTUAL == ((lb)->type))
/*****************************************************************************
 ...
 *
 *****************************************************************************/
bl_Return_t Lbm_ReadFingerprint(bl_Buffer_t *buffer,
                                      bl_BufferSize_t *respSize)
{
    ...
    for(i = 0;i < (LBM_NUMBER_OF_LOGICAL_BLOCK
                   - LBM_NUMBER_OF_DRIVER
                   - LBM_NUMBER_OF_VIRTUAL);i ++)
    {
        block = LBM_GET_LBPTR(i);

        ret = _Lbm_ReadFingerprint(block);
        if(BL_ERR_OK != ret)
        {
            break;
        }

        *respSize = (bl_BufferSize_t)(RTE_FINGERPRINT_SIZE + 1) * (i + 1);

        buffer[0 + (RTE_FINGERPRINT_SIZE + 1) * i] = i;
        Bl_MemCpy(&buffer[1 + (RTE_FINGERPRINT_SIZE + 1) * i],
                    gs_LbmFingerprint, RTE_FINGERPRINT_SIZE);
    }
...
/**************************************************************************//**
 *
 *  \details Get the header of a logical block.
 *
 *  \param[in]  id - the index of the logical block array.
 *                  header - the header of the logical block.
 *
 *  \since  V3.0.0
 *
 *****************************************************************************/
bl_Return_t Lbm_GetHeader(bl_u16_t id,
                                bl_LogicalBlockHeader_t *header)
{
    const bl_LogicalBlock_t *block;
    bl_Return_t ret = BL_ERR_NOT_OK;
    ...
    if (LBM_IS_VALID_STATUS(block) || LBM_IS_UPDATED_STATUS(block))
    {
        ret = Dm_IsValidDataId(block->headerId);
        if (BL_ERR_OK == ret)
        {
            ret = Dm_ReadData(block->headerId,
                              RTE_FILEHEADER_SIZE,
                              (bl_Buffer_t *)header);
        }
    }
...
static bl_Return_t _Lbm_TryToClearValidFlag(const bl_LogicalBlock_t *lb)
{
    ...
    {
        ret = Mem_Erase(Dm_GetMemid(lb->validFlagId),
                        Dm_GetDataAddr(lb->validFlagId),
                        Dm_GetDataSize(lb->validFlagId));
    }
    return ret;
}
#endif
...
static bl_Return_t _Lbm_CalculateHashByDct(const bl_DownContext_t *dct)
{
    ...
    for (i = 0; i < dct->segIndex; i++)
    {
        ...
        do
        {
            ...
            ret = Image_UpdateChecksum(calAddr, readSize, g_HashCalculation);
            if (ret != BL_ERR_OK)
            {
                break;
            }
...
/**************************************************************************//**
 *
 *  \details Read the contents of the buffer to ROM.
 *
 *  \param[in]  block - the pointer of the logical block.
 *
 *  \retval BL_ERR_OK       - the fingerprint are written to the ROM.
 ...
 *
 *****************************************************************************/
bl_Return_t _Lbm_ReadFingerprint(const bl_LogicalBlock_t *block)
{
    bl_Return_t ret = BL_ERR_NOT_OK;
...
bl_Return_t Lbm_ReadDownloadDataandCalCheckSum( bl_Address_t address,
												bl_Size_t size,
												bl_u16_t *checksum)
{
	bl_u8_t buffer[512] = {0};//Max size 512
	bl_u16_t i;
    bl_Return_t ret = BL_ERR_NOT_OK;

   if(size <= 512)
   {
		ret = Mem_Read(1,address,size,buffer);//App flash
		g_ImgSecOps.encryption(address, buffer, size);
		if(BL_ERR_OK == ret)
		{
			for(i = 0; i < size; i++)
			{
