...
void Adpt_InitDelayInfo(bl_SecurityDelay_t *delay)
{
    ...
    {
        delay->delayTime    = 1000u;//1000ms
        delay->attemptCount = g_SecurityDelayInfo.attemptCount;
    }
#else
if(startup_flag == 0)
{
    delay->delayTime    = 1000u;//boot startup 1s
    delay->attemptCount = g_SecurityDelayInfo.attemptCount;
    startup_flag = 1;
}
else
{
    delay->delayTime    = g_SecurityDelayInfo.delayTime;//boot startup 1s
    delay->attemptCount = g_SecurityDelayInfo.attemptCount;
}
...
bl_Return_t Adpt_CheckCompatibility(void)
{
    ...
    if(BL_ERR_OK == ret) /*lint !e774*//*PRQA S 3355,3358*/
    {
        ret = Lbm_IsAllValid();
        //OK => BL_ERR_OK
        //NOT_OK => BL_ERR_NOT_OK
    }

    /*save status into nonvolatile memory*/
    ret = Lbm_SaveAllStatus();
