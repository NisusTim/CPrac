...
/** \brief Indicate the 31 0202 service is not ok.*/
#define ADPT_ROUTINE_INCORRECT_RESULT_IMG_SECURITY_CHECKSUM_ERROR   (2)
...
//static void _Adpt_UdsResetSystem(bl_u8_t sid, bl_u16_t flagId);   //Remove by Wood, in order to resolve unused warning
...
static bl_u16_t gs_DownloadCheckSum;

static bl_u16_t gs_ReceiveCheckSum;
...
static const bl_Buffer_t gs_HardResetResponse[ADPT_UDS_CAN_FRAME_INDEX_3] =
{
    0x02u,0x51u,0x01u
};

static const bl_Buffer_t gs_SoftResetResponse[ADPT_UDS_CAN_FRAME_INDEX_3] =
{
    0x02u,0x51u,0x03u
};
...
bl_Return_t Adpt_UdsInit(void)
{
    ...
    gs_DownloadCheckSum = 0;
    gs_ReceiveCheckSum = 0;
    return ret;
}
                                       ...
                                       bl_BufferSize_t *respSize)
{
	...
	Secm_StartHash(); //Avoid the program interrupt like no power or line disconnection.
                                       ...
                                       bl_BufferSize_t *respSize)
{
    ...
    if (Dcm_IsSuspendedResponse() != BL_ERR_OK)
    {
        _Adpt_SendResponse((bl_BufferSize_t)sizeof(gs_HardResetResponse),
        		gs_HardResetResponse);
    }
...
/**************************************************************************//**
 *
 *  \details Callback of reading F180.
 *
 *  \param[in]     size     - the data size, not include id, subid and did.
 *  \param[in/out] buffer   - the pointer of data.
 *  \param[out]    respSize - the size of response data.
 ...
 *
 *****************************************************************************/
bl_ResponseCode_t Adpt_UdsCallbackReadF180(bl_BufferSize_t size,
                                                  bl_Buffer_t *buffer,
                                                  bl_BufferSize_t *respSize)
{
	...
	bl_u8_t* temp = get_F180_BootloaderVersionNumber();

	for(i = 0; i < ADPT_F180_SIZE ; i++)
	{
		buffer[i] = temp[i];
	}

	*respSize = ADPT_F180_SIZE;
	return resCode;
}
...
{
	bl_ResponseCode_t resCode = DCM_E_POSITIVERESPONSE;
	bl_u8_t F187[] = {"27070LA110"};
	bl_u8_t i;
	(void)size;

	for(i=0;i<ADPT_F187_SIZE;i++)
	{
		if(i<(sizeof(F187) - 1))
		{
			buffer[i] = F187[i];
                                                      ...
                                                      bl_BufferSize_t *respSize)
{
	...
	bl_ResponseCode_t resCode = DCM_E_POSITIVERESPONSE;
	bl_u8_t F18C[] = {"28084UA105"};
	bl_u8_t i;
	(void)size;

	for(i=0;i<ADPT_F18C_SIZE;i++)
	{
		if(i<(sizeof(F18C) - 1))
		{
			buffer[i] = F18C[i];
...
{
	bl_ResponseCode_t resCode = DCM_E_POSITIVERESPONSE;
	bl_u8_t F193[] = {"93G007BV3.2"};
	bl_u8_t i;
	(void)size;

	for(i=0;i<ADPT_F193_SIZE;i++)
	{
		if(i<(sizeof(F193) - 1))
		{
			buffer[i] = F193[i];
...
/**************************************************************************//**
 *
 *  \details Callback of reading F198.
 *
 *  \param[in]     size     - the data size, not include id, subid and did.
 *  \param[in/out] buffer   - the pointer of data.
 *  \param[out]    respSize - the size of response data.
 ...
 *
 *****************************************************************************/
bl_ResponseCode_t Adpt_UdsCallbackReadF198(bl_BufferSize_t size,
                                                      bl_Buffer_t *buffer,
                                                      bl_BufferSize_t *respSize)
{
	bl_ResponseCode_t resCode = DCM_E_POSITIVERESPONSE;
	...
	bl_u8_t* temp = get_F198_TesterSeriesNumber();

	for(i = 0; i < ADPT_F198_SIZE ; i++)
	{
		buffer[i] = temp[i];
	}

	*respSize = ADPT_F198_SIZE;
	return resCode;
}

/**************************************************************************//**
 *
 *  \details Callback of reading F199.
 *
 *  \param[in]     size     - the data size, not include id, subid and did.
 *  \param[in/out] buffer   - the pointer of data.
 *  \param[out]    respSize - the size of response data.
 ...
 *
 *****************************************************************************/
bl_ResponseCode_t Adpt_UdsCallbackReadF199(bl_BufferSize_t size,
                                                      bl_Buffer_t *buffer,
                                                      bl_BufferSize_t *respSize)
{
    bl_ResponseCode_t resCode = DCM_E_POSITIVERESPONSE;
	...
	bl_u8_t* temp = get_F199_ProgrammingDate();

	for(i = 0 ; i < ADPT_F199_SIZE ; i++)
	{
		buffer[i] = temp[i];
	}

	*respSize = ADPT_F199_SIZE;
    return resCode;
}
...
/**************************************************************************//**
 *
 *  \details Callback of writting F198.
 *
 *  \param[in]     size     - the data size, not include id, subid and did.
 *  \param[in/out] buffer   - the pointer of data.
 *  \param[out]    respSize - the size of response data.
 ...
 *****************************************************************************/
/*lint -e{818}*/
bl_ResponseCode_t Adpt_UdsCallbackWriteF198(bl_BufferSize_t size,
                                                   bl_Buffer_t *buffer,
                                                   bl_BufferSize_t *respSize)
{
    ...
    *respSize = 0u;
	...
	if(BL_ERR_OK == set_F198_TesterSeriesNumber(temp))
	{
		resCode = DCM_E_POSITIVERESPONSE;
	}
	else
...
/**************************************************************************//**
 *
 *  \details Callback of writting F199.
 *
 *  \param[in]     size     - the data size, not include id, subid and did.
 *  \param[in/out] buffer   - the pointer of data.
 *  \param[out]    respSize - the size of response data.
 ...
 *****************************************************************************/
/*lint -e{818}*/
bl_ResponseCode_t Adpt_UdsCallbackWriteF199(bl_BufferSize_t size,
                                                   bl_Buffer_t *buffer,
                                                   bl_BufferSize_t *respSize)
{
	 bl_ResponseCode_t resCode = DCM_E_GENERALPROGRAMMINGFAILURE;
	bl_u8_t i;
	bl_u8_t temp[ADPT_F199_SIZE] = {0};
	...
	*respSize = 0u;
	...
	if(BL_ERR_OK == set_F199_ProgrammingDate(temp, ADPT_F199_SIZE))
	{
		resCode = DCM_E_POSITIVERESPONSE;
	}
	else
                                           ...
                                           bl_BufferSize_t *respSize)
{
    ...
    if(gs_ReceiveCheckSum == gs_DownloadCheckSum)
    {
    	gs_ReceiveCheckSum = 0;
    	gs_DownloadCheckSum = 0;
        ...
        {
            buffer[0] = ADPT_ROUTINE_INCORRECT_RESULT_IMG_SECURITY_CHECKSUM_ERROR;
        }
    }
    else
    {
                                     ...
                                     bl_BufferSize_t *respSize)
{
    ...
    do
    {
        ...
        ret = Lbm_ReadDownloadDataandCalCheckSum(gs_UdsPrivateData.address,
												 dataSize,
												 &gs_DownloadCheckSum);
...
//TODO :: Need Implement CheckSum
    ...
    else
    {
        Rte_ClearDownStatus(&g_DownContext,
                                ...
                                | ADPT_STATUS_CONFIRM_TRANS_DATA));
//        buffer[0]=(bl_u8_t)((gs_DownloadCheckSum >> 8) & 0x00FF);
//        buffer[1]=(bl_u8_t)(gs_DownloadCheckSum & 0x00FF);
        *respSize = 0x00u;
    }
...
static void _Adpt_SendResponse(bl_BufferSize_t size, const bl_Buffer_t *buffer)
{
    ...
    bl_Buffer_t temp_buffer[ADPT_MAX_FRAME_SIZE] = {0};
    /*whether return value is necessary*/

    Bl_MemCpy(temp_buffer,buffer,size);
    (void)Canif_Write(ADPT_UDS_TX_HANDLE, ADPT_MAX_FRAME_SIZE, temp_buffer);
