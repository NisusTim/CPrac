 ...
 *****************************************************************************/
#if(ADPT_FUN_CHECK_COMPATIBILITY == BL_FUN_ON)
 ...
 *****************************************************************************/
#if(ADPT_FUN_CHECK_COMPATIBILITY == BL_FUN_ON)
#define FILE_QUANTITY                           (LBM_NUMBER_OF_LOGICAL_BLOCK \
                                                 - LBM_NUMBER_OF_DRIVER \
                                                 - LBM_NUMBER_OF_VIRTUAL)
#define HEADER_VER_BT                           (0x42547631UL)
#endif
/*****************************************************************************
...
/**************************************************************************//**
 *
 *  \details 1.It is necessary to call Lbm_ClearStatus(LogicalBlockList_ID),
 *             when the compatibility check is not passed.
 *           2.LogicalBlockList_ID is the corresponding index of logical block
 *             failed in the compatibility check.
 *  \since  V2.0.0
 *
 *****************************************************************************/
bl_Return_t App_CheckCompatibility(void)
{
    ...
    bl_LogicalBlockHeader_t header[FILE_QUANTITY];
    bl_u8_t i;

    for(i = 0; i < FILE_QUANTITY; i++)
    {
        (void)Lbm_GetHeader(i, &header[i]);
        if(HEADER_VER_BT != header[i].verBT)
        {
            (void)Lbm_ClearStatus(i);
            ret = BL_ERR_NOT_OK;
        }
    }

#if(FILE_QUANTITY > 1u)
    for(i = 1; i < FILE_QUANTITY; i++)
    {
        if((0u != header[i].verAPP)&&(header[0].verAPP != header[i].verAPP))
        {
            (void)Lbm_ClearStatus(i);
            ret = BL_ERR_NOT_OK;
        }
