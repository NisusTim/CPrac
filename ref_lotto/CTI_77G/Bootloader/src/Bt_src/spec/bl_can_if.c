...
static void _Canif_SetRxCanIdInCanParameter(bl_u32_t id);
static void _Canif_SetTxCanIdInCanParameter(bl_u32_t id);
#endif
...
#if (CANIF_FUN_CANID_IN_PARAMETERS == BL_FUN_ON)
        pdu.canId = Canif_GetTxCanIdInCanParameter();
#else
...
void Canif_SetTxCanId(bl_ComIfHandle_t handle, bl_u32_t id)
{
    if (handle < g_CanIfTxPduCfg.number)
    {
        if(id != g_CanIfTxPduCfg.txList[handle].id)
        {
            g_CanIfTxPduCfg.txList[handle].id = id;

#if (CANIF_FUN_CANID_IN_PARAMETERS == BL_FUN_ON)
            _Canif_SetTxCanIdInCanParameter(id);
#endif
        }
    }
...
void Canif_SetRxCanId(bl_ComIfHandle_t handle, bl_u32_t id)
{
    ...
    {
        if(id != g_CanIfRxPduCfg.rxList[handle].id)
        {
            g_CanIfRxPduCfg.rxList[handle].id = id;

#if (CANIF_FUN_CANID_IN_PARAMETERS == BL_FUN_ON)
            _Canif_SetRxCanIdInCanParameter(id);
#endif
        }
    }
...
static void _Canif_SetRxCanIdInCanParameter(bl_u32_t id)
{
    Can_SetUdsPhysicalRequestId(id);
}

static void _Canif_SetTxCanIdInCanParameter(bl_u32_t id)
{
    Can_SetUdsPhysicalResponseId(id);
}

bl_u32_t Canif_GetRxCanIdInCanParameter()
{
    bl_u32_t id;
    id = Can_GetUdsPhysicalRequestId();
    return id;
}

bl_u32_t Canif_GetTxCanIdInCanParameter()
{
    bl_u32_t id;
    id = Can_GetUdsPhysicalResponseId();
    return id;
}
#endif
...
static bl_Return_t _Canif_Read(bl_CanRxPdu_t *pdu)
{
    ...
    {
        if(pdu->canId == CANIF_PHY_M_RX_CANID)
        {
            Canif_SetRxCanId(0U, CANIF_PHY_M_RX_CANID);
            Canif_SetTxCanId(0U, CANIF_PHY_M_TX_CANID);
        }
        else if(pdu->canId == CANIF_PHY_C_RX_CANID)
        {
            Canif_SetRxCanId(0U, CANIF_PHY_C_RX_CANID);
            Canif_SetTxCanId(0U, CANIF_PHY_C_TX_CANID);
        }
        else
        {
            Canif_SetRxCanId(0U, CANIF_PHY_M_RX_CANID == g_CanIfRxPduCfg.rxList[0].id ? CANIF_PHY_M_RX_CANID : CANIF_PHY_C_RX_CANID);
            Canif_SetTxCanId(0U, CANIF_PHY_M_RX_CANID == g_CanIfRxPduCfg.rxList[0].id ? CANIF_PHY_M_TX_CANID : CANIF_PHY_C_TX_CANID);
        }
