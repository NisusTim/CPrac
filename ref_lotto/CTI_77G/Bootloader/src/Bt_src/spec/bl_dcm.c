...
#define DCM_SUSPEND_POSRSP_MASK             (0x7Fu)
...
void Dcm_TimerFunction(void)
{
    if(DCM_IS_STARTED_S3TIMER())
    {
        ...
        {
            //Rte_ReportEvent(RTE_EVENT_TYPE_S3TIME_TIMEOUT);//No Reset
        }
    }
...
/**************************************************************************//**
 *
 *  \details Dcm clear supress positive response bit.
 *
 *  \since  V5.1.0
 *
 *****************************************************************************/
void Dcm_Allow_Positive_Response(void)
{
    DCM_ALLOW_POSITIVE_RESPONSE();
}
...
static bl_BufferSize_t _Dcm_MakeSubId(bl_Buffer_t *buffer, bl_u16_t *id)
{
    ...
    {
        buffer[0] = (bl_Buffer_t)(buffer[0] & DCM_SUSPEND_POSRSP_MASK);
