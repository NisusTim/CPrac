...
static Fls_SectorSlecType FlashDri_SectorSelect(bl_FlashDrvParam_t* fp);
...
__attribute__ ((section(".fls_header")))
bl_FlashDrvHeader_t g_FlashHeader =
{
...
__attribute__ ((section(".fls_array")))
static Fls_SectorInfoType gs_FlsDriSectInfoArray[FLSDRI_SECTOR_MAX_NUM]=
{
     {CFLSDRI_ADDRESS_START_L2, CFLSDRI_ADDRESS_END_L2, CFLSDRI_SECTOR4_L2, FLASHDRI_LOW_BLOCK},
     {CFLSDRI_ADDRESS_START_L3, CFLSDRI_ADDRESS_END_L3, CFLSDRI_SECTOR5_L3, FLASHDRI_LOW_BLOCK},
     {CFLSDRI_ADDRESS_START_H0, CFLSDRI_ADDRESS_END_H0, CFLSDRI_SECTOR6_H0, FLASHDRI_MID_BLOCK},
     {CFLSDRI_ADDRESS_START_H1, CFLSDRI_ADDRESS_END_H1, CFLSDRI_SECTOR7_H1, FLASHDRI_MID_BLOCK},
     {CFLSDRI_ADDRESS_START_H2, CFLSDRI_ADDRESS_END_H2, CFLSDRI_SECTOR8_H2, FLASHDRI_MID_BLOCK},
     {CFLSDRI_ADDRESS_START_H3, CFLSDRI_ADDRESS_END_H3, CFLSDRI_SECTOR9_H3, FLASHDRI_MID_BLOCK},
     {CFLSDRI_ADDRESS_START_H4, CFLSDRI_ADDRESS_END_H4, CFLSDRI_SECTOR10_H4, FLASHDRI_MID_BLOCK},
     {CFLSDRI_ADDRESS_START_H5, CFLSDRI_ADDRESS_END_H5, CFLSDRI_SECTOR11_H5, FLASHDRI_MID_BLOCK},
     {CFLSDRI_ADDRESS_START_B0, CFLSDRI_ADDRESS_END_B0, CFLSDRI_SECTOR12_B0, FLASHDRI_MH_BLOCK},
     {CFLSDRI_ADDRESS_START_B1, CFLSDRI_ADDRESS_END_B1, CFLSDRI_SECTOR13_B1, FLASHDRI_MH_BLOCK},
     {CFLSDRI_ADDRESS_START_B2, CFLSDRI_ADDRESS_END_B2, CFLSDRI_SECTOR14_B2, FLASHDRI_MH_BLOCK},
     {CFLSDRI_ADDRESS_START_B3, CFLSDRI_ADDRESS_END_B3, CFLSDRI_SECTOR15_B3, FLASHDRI_MH_BLOCK},
     {CFLSDRI_ADDRESS_START_B4, CFLSDRI_ADDRESS_END_B4, CFLSDRI_SECTOR16_B4, FLASHDRI_MH_BLOCK},
     {CFLSDRI_ADDRESS_START_B5, CFLSDRI_ADDRESS_END_B5, CFLSDRI_SECTOR17_B5, FLASHDRI_MH_BLOCK},
};

/*********************************************************************************************************************/
/* DriverCode                                                                                                        */
/*********************************************************************************************************************/
static bl_u8_t init_flag = 0;
__attribute__ ((section(".fls_code")))
void  FlashDriInit(bl_FlashDrvParam_t* fp)
{
    /* unlock  blocks*/
...
END_FUNCTION_HDR */
__attribute__ ((section(".fls_code")))
void  FlashDriDeinit(bl_FlashDrvParam_t* fp)
{
    /*disable erass or program */
    WRITE32(FLS_BASE_ADDRESS + C55FMC_C_LOCK0, 0xFFFFFFFF);
    WRITE32(FLS_BASE_ADDRESS + C55FMC_C_LOCK1, 0xFFFFFFFF);
...
END_FUNCTION_HDR */
__attribute__ ((section(".fls_code")))
void  FlashDriErase(bl_FlashDrvParam_t* fp)
{
    Fls_SectorSlecType temp;
    FS8x_Set_Last_Feed_Timestamp(fp->fs8x_wd_ftime);
    ...
    else if((fp->address < FLASHDRI_START_ADRESS)
         || (fp->address > FLASHDRI_END_ADRESS))
    {
        /* Address error*/
        fp->errCode = BL_ERR_NOT_OK;
    }
    ...
    fp->fs8x_wd_ftime = FS8x_Get_Last_Feed_Timestamp();
}
...
END_FUNCTION_HDR */
__attribute__ ((section(".fls_code")))
void  FlashDriWrite(bl_FlashDrvParam_t* fp)
{
	FS8x_Set_Last_Feed_Timestamp(fp->fs8x_wd_ftime);
    ...
    else
    {
        /* Set MCR PGM bit */
        SET32(FLS_BASE_ADDRESS, C55FMC_MCR_PGM);
        //WRITE64(fp->address, *((bl_u64_t *)fp->wrdata));
        WRITE64(fp->address,fp->wrdata);
...
static Fls_SectorSlecType FlashDri_SectorSelect(bl_FlashDrvParam_t* fp)
{
     ...
     for(sct=0; sct<FLSDRI_SECTOR_MAX_NUM; sct++)
     {
    	 FS8x_Feed();
    	 SWT_0.SR.R = 0xA602;
    	 SWT_0.SR.R = 0xB480;

         if ((addr >= gs_FlsDriSectInfoArray[sct].BlockAddressStart) &&
            (addr <= gs_FlsDriSectInfoArray[sct].BlockAddressEnd))
         {
             temp.SectorId = gs_FlsDriSectInfoArray[sct].SectorId;
             temp.Blocktype = gs_FlsDriSectInfoArray[sct].Blocktype;
             break;
         }
     }
