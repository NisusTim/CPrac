...
{
    bl_Return_t ret;
    bl_u8_t FingerDataflg[1] = {0};

    ret = Dm_IsDefaultData(BOOTM_REPROGRAM_FLAG_DID);
    if (BL_ERR_OK == ret)
    {
        //ret = Dm_ReadData(BOOTM_FINGER_FLAG_DID,
                      //    Dm_GetDataSize(BOOTM_FINGER_FLAG_DID),
                       //   &FingerDataflg[0]);
        //if (BL_ERR_OK == ret)
        {

            ret = Mem_Erase(Dm_GetMemid(BOOTM_REPROGRAM_FLAG_DID),
                            Dm_GetDataAddr(BOOTM_REPROGRAM_FLAG_DID),
                            Dm_GetDataSize(BOOTM_REPROGRAM_FLAG_DID));
            //if (BL_ERR_OK == ret)
            {
            
                //ret = Dm_WriteData(BOOTM_FINGER_FLAG_DID,
                                //   Dm_GetDataSize(BOOTM_FINGER_FLAG_DID),
                                  // &FingerDataflg[0]);
                //if (BL_ERR_OK == ret)
                {
