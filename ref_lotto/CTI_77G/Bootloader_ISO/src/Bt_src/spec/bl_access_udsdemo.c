...
{
    bl_UdsDemoSeed_t UdsDemoSeedDummy;

    UdsDemoSeedDummy = ((bl_UdsDemoSeed_t)g_dummy) << 16u;
    UdsDemoSeedDummy |= g_dummy;

    gs_UdsDemoSeed *= UdsDemoSeedDummy;
...
static bl_UdsDemoKey_t _Acc_UdsDemoCalculateKey(void)
{
    ...
    bl_u8_t lkey[4];
    bl_u8_t Seed[4];
    bl_u8_t Xor[4] = {0xCD, 0x11, 0xE9, 0x56};
    bl_u8_t Cal[4];
    bl_u8_t i;
    ...
    lkey[0] = ((Cal[2] & 0x0Fu) << 4u) | (Cal[1] & 0x0Fu);
    lkey[1] = ((Cal[3] & 0x0Fu) << 4u) | ((Cal[1] & 0xF0u) >> 4);
    lkey[2] = (Cal[0] & 0xF0u) | ((Cal[2] & 0xF0u) >> 4u);
    lkey[3] = ((Cal[0] & 0x0Fu) << 4u) | (Cal[3] & 0x0Fu);

    key = BL_BE32_TO_MCU(lkey);
