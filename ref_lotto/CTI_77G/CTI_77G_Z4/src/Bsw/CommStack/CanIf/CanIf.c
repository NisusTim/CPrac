...
//STATIC FUNC(Std_ReturnType, CANIF_PRIVATE_CODE) CanIf_RxParCheck      //Remove by Wood, in order to resolve never defined warning
//(
...
FUNC(void, CANIF_PUBLIC_CODE) CanIf_Init(void)
{
    ...
    // set the UDS physical request/response address from canp parameter
    //CanIfRxPduConfig[0].CanId = get_uds_physical_request();
    CanIfTxPduConfig[0].CanId = get_uds_physical_response();
...
/* BEGIN_FUNCTION_HDR
********************************************************************************
* Function Name: CanIf_Transmit_Ready_Test
*
* Description:   This service test a can PDU transmit ready state
*
*
...
********************************************************************************
END_FUNCTION_HDR*/
FUNC(Std_ReturnType, CANIF_PUBLIC_CODE) CanIf_Transmit_Ready_Test
(
...
)
{
	...
	if(DYNAMIC_TXPDU == CanIfTxPduConfig[CanTxPduId].TxPduType)
	{
		hth = dynamic_hth;
		hth++;
		/*Confirm the default and minimum value*/
		if(hth < CanIfTxPduConfig[CanTxPduId].Hth)
		{
        ...
        else
        {
            ...
            #endif
            {
                ...
                if(DYNAMIC_TXPDU == CanIfTxPduConfig[CanTxPduId].TxPduType)
                {
                	...
                	/*Confirm the default and minimum value*/
					if(dynamic_hth < CanIfTxPduConfig[CanTxPduId].Hth)
					{
						dynamic_hth = CanIfTxPduConfig[CanTxPduId].Hth;
					}

					/*Confirm the maximum value*/
                	if(dynamic_hth >= CAN_USED_HOH_NUM)
					{
						dynamic_hth = CanIfTxPduConfig[CanTxPduId].Hth;
					}
					hth = dynamic_hth;
				...
				hth = CanIfTxPduConfig[CanTxPduId].Hth;
#endif
                CanTxResult = Can_Write((hth), &CanIfTxPduObject);
...
)
{
...

    {
        ...
        #endif
        {
			if((RMTP_DATA_TXPDUID == CanTxPduId) || (IPDU_DATA_TXPDUID == CanTxPduId))
			{
				App_ComTx_Confirmation(CanTxPduId);
			}
...
)
{
    ...
    // set the UDS physical response address from CanID
    if(CanId != get_uds_physical_request())
    {
        if(CanId == 0x7B0)
        {
            set_uds_physical_request(0x7B0);
            set_uds_physical_response(0x7B8);
            can_info_pars_save_handler();
            CanIfTxPduConfig[0].CanId = 0x7B8;

        }
        else if (CanId == 0x7A0)
        {
            set_uds_physical_request(0x7A0);
            set_uds_physical_response(0x7A8);
            can_info_pars_save_handler();
            CanIfTxPduConfig[0].CanId = 0x7A8;
        }
        else if (CanId == 0x7C1)
        {
            set_uds_physical_request(0x7C1);
            set_uds_physical_response(0x7C9);
            can_info_pars_save_handler();
            CanIfTxPduConfig[0].CanId = 0x7C9;
        }
    }
