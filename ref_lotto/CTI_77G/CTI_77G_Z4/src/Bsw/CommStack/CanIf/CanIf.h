...
#define CANIF_ENTER_CRITICAL_SECTION()          Can_DisableGlobalInterrupt()
#define CANIF_LEAVE_CRITICAL_SECTION()          Can_EnableGlobalInterrupt()
 ...
 *******************************************************************************/
typedef enum {
    CANIF_UNINITIALIZED = 0, CANIF_INITIALIZED = 1
} CanIf_InitType;

typedef enum {
    STATIC_TXPDU = 0, DYNAMIC_TXPDU = 1
} CanIf_TxPduType;

typedef enum {
    STANDARD_CAN = 0, EXTENDED_CAN = 1
} CanIf_PduCanIdType;
...
extern VAR(boolean, CANIF_PRIVATE_DATA)
CanIfTxBufferPDUFillflag[];
extern VAR(uint8, CANIF_PRIVATE_DATA)
CanIfTxBufferSdu[][CAN_DATA_DLC];
extern VAR(Can_PduType, CANIF_PRIVATE_DATA)
CanIfTxBufferPdu[];

#endif

#if (STD_ON == CANIF_PUBLIC_SETDYNAMICTXID_API)
extern VAR(Can_IdType, CANIF_PRIVATE_DATA)
CanIfTxPduCanIdForDynamic[];
#endif

#if(STD_ON == CANIF_PUBLIC_READTXPDU_NOTIFY_STATUS_API)
extern VAR(CanIf_NotifStatusType, CANIF_PRIVATE_DATA)
CanIfTxNotifStatus[];
#endif
...
extern VAR(uint8, CANIF_PRIVATE_DATA)
CanIfRxBufferSduData[][CAN_DATA_DLC];
extern VAR(PduLengthType, CANIF_PRIVATE_DATA)
CanIfRxBufferSduLength[];
...
extern CONST(PduIdType, CANIF_CONFIG_CONST)
CanIfInitNumberOfCanRxPduIds;
extern CONST(PduIdType, CANIF_CONFIG_CONST)
CanIfInitNumberOfCanTxPduIds;
extern CONST(Can_HwHandleType, CANIF_CONFIG_CONST)
CanIfInitNumberOfHrh;
...
#if(CANIF_FILTER_TYPE_INDEX == CANIF_PRIVATE_SOFTWARE_FILTER_TYPE)
extern CONST(Can_HwHandleType, CANIF_CONFIG_CONST)
CanIfHrhFirstRxPduID[];
extern CONST(Can_HwHandleType, CANIF_CONFIG_CONST)
CanIfHthFirstTxPduID[];
#endif

#if(STD_ON == CANIF_HRH_RANGE_FILTER)
extern CONST(CanIf_HrhRangeCfgType, CANIF_CONFIG_CONST)
CanIfHrhRangeCfg[];
#endif

extern VAR(CanIf_RxPduConfigType, CANIF_PRIVATE_DATA)
CanIfRxPduConfig[];
extern VAR(CanIf_TxPduConfigType, CANIF_PRIVATE_DATA)
CanIfTxPduConfig[];
...
extern FUNC(void, CANIF_PUBLIC_CODE)
CanIf_Init(void);

extern FUNC(Std_ReturnType, CANIF_PUBLIC_CODE)
CanIf_SetControllerMode(uint8 ControllerId,
...
#if(STD_ON == CANIF_PUBLIC_READTXPDU_NOTIFY_STATUS_API)
extern FUNC(CanIf_NotifStatusType, CANIF_PUBLIC_CODE)
...
extern FUNC(void, CANIF_PUBLIC_CODE)
CanIf_ClearTransmitBuffer(uint8 ControllerId);
