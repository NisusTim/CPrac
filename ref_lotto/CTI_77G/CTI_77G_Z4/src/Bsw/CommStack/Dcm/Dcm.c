...
VAR(Dcm_MsgContextType, DCM_NOINIT_DATA) gMsgContextBackup;
...
//STATIC FUNC(void,DCM_PRIVATE_CODE)  DsdInternal_Periodic_Transmit(void);      //Remove by Wood, in order to resolve never defined warning
...
*                
* Description:   Implementation of UDS service 0x14
*                             
* Inputs:        None
*                
* Outputs:       None
...
*                
* Description:   Implementation of UDS service 0x19
*                             
* Inputs:        None
*                
* Outputs:       None
...
*                
* Description:   Implementation of UDS service 0x23
*                             
* Inputs:        None
*                
* Outputs:       None
...
*                
* Description:   Implementation of UDS service 0x24
*                             
* Inputs:        None
*                
* Outputs:       None
...
*                
* Description:   Implementation of UDS service 0x27
*                             
* Inputs:        None
*                
* Outputs:       None
...
)
{
    ...
    if((gMsgContextType.reqData[DCM_INDEX_1]&1u) != DCM_SERVICE_27_SEND_KEEY) 
    {
        ...
        /* Check the requested security level has already been cracked, if so, send zero seeds */
        ...
        #endif
        /* If not, call related application callback function to get the seeds */
        else 
        {
            /*gCurrentSecurityIndex = gSecLevelForCurrentSecurityAccessRequestLevel;*/
            gSecurityAcessSequence[gCurrentSecurityIndex] = HAVE_GOTTON_SEED;
    ...
    else
    {
        ...
        {   
            ...
            {
                /* Call related application callback function to compare the key */
                gSubFunctionHandler = gDcmDsdSubService[gSubServicePosition].DcmDsdSubFunction;
                if(gSubFunctionHandler != DCM_NULL)
                {
                    (*gSubFunctionHandler)(&gMsgContextType);
...
*                
* Description:   Implementation of UDS service 0x28
*                             
* Inputs:        None
*                
* Outputs:       None
...
)
{
    ...
    /* Check communication type ,only support both normal and NM type*/
    else if((gMsgContextType.reqData[DCM_INDEX_2]& BOTH_NORMAL_AND_NM_MSG) != 3u)
    {
        DsdInternal_SetNegResponse(&gMsgContextType,DCM_E_REQUESTOUTOFRANGE);
    }
    else
...
*                
* Description:   Implementation of UDS service 0x2A
*                             
* Inputs:        None
*                
* Outputs:       None
...
)
{
    ...
    /* removed temp */
    /* constant static PeriodicDidlistType CurrentPeriodicDid;*/

    /* initial temp_periodicdidlist[0] as flag */
            ...
            for(i = (uint32)0u; i < (gMsgContextType.reqDataLen - (uint32)DCM_REQUIRE_DATA_LENGTH_2_BYTE); i++)
            {
                for(j = (uint32)0u; j < (uint32)NUMBER_OF_PERIODIC_DID; j++)
                {
                    if(gMsgContextType.reqData[(uint32)DCM_REQUIRE_DATA_LENGTH_2_BYTE + i] == gDcmDsdSubService_2A[j].DcmDspDidIdentifier)
                    {
                        ...
                        {
                            /* AddressingMode is not supported */
                            DsdInternal_SetNegResponse(&gMsgContextType,DCM_E_CONDITIONSNOTCORRECT);
                            error = DCM_HAVE_ERRORS;
                            j = (uint32)NUMBER_OF_PERIODIC_DID;
                            i = gMsgContextType.reqDataLen;
            ...
            else
            {
                if(gMsgContextType.reqData[DCM_INDEX_1] < DCM_INDEX_4)
                {
                    ...
                    for(i = (uint32)0u; i < counter; i++)
                    {
                        ...
                        {
                            /* change rate from next periodic ,so delete */
                        }
                        else
                        {
                ...
                { 
                    /* delete DID from  Periodic Tx table (PeriodicDidList)*/
                    ...
                    if(error == DCM_HAVE_NO_ERRORS)
                    {
                        ...
                        for(j = (uint32)0u; j < (uint32)PeriodicDidCounter; j++)
                        {
                            ...
                            if(DCM_UINT8_0 == PeriodicDidList[j].DidPosition) 
                            {
                                 ...
                                 * if the current did is not delete
                                 */
    ...
    /* No error happened,positive response */
    if(error == DCM_HAVE_NO_ERRORS) 
    {
        gMsgContextType.resDataLen = (uint32)1u;
        DsdInternal_ProcessingDone(&gMsgContextType);
...
/* Assumption: the addresssAndLength format is the same as 0x23 and 0x3d */
...
)
{
    ...
    else
    {
        ...
        for(i = 0u; i < NUMBER_OF_DYN_DID; i++)
        {
            ...
            {
                /* Check the security access condition of dynamic did */
            ...
            if(gMsgContextType.reqData[DCM_INDEX_1] == 0x01u) 
            {
                ...
                else
                {
                    ...
                    if(error == STD_OFF)
                    {
                        ...
                        {
                            /* No error happened,move all the source Did info in to global variable gDynamicalDidRelatedInfo[8] */
            ...
            else if(gMsgContextType.reqData[DCM_INDEX_1] == DCM_SERVICE_2C_DEFINE_BY_MEMORY_ADDRESS)
            {
                ...
                {   
                    ...
                    if(error == STD_OFF)
                    {
                        ...
                        {
                            /* 
                             * No error happened,move all the source did info in to global
...
*                
* Description:   Implementation of UDS service 0x2E
*                             
* Inputs:        None
*                
* Outputs:       None
...
*                
* Description:   Implementation of UDS service 0x2F
*                             
* Inputs:        None
*                
* Outputs:       None
...
*                
* Description:   Implementation of UDS service 0x31
*                             
* Inputs:        None
*                
* Outputs:       None
...
)
{
	...
	/*minimum length check, at least 4(SI+SubFunction+RID Parameter)*/
    if(gMsgContextType.reqDataLen < 4)
    {
        DsdInternal_SetNegResponse(&gMsgContextType,DCM_E_INCORRECTMESSAGELENGTHORINVALIDFORMAT);
    }
    ...
    {   
        ...
        if(gMsgContextType.reqData[DCM_INDEX_1] == START_ROUTINE)
        {
            ...
            else
            {
                ...
                { 
                    ...
                    {    
                        /* Sub-Function Execution state check */
                        /* is  Sub-Function supported in session  */
                        /* all condition have checked and no error*/
                        /* Check service id addressing mode  */
                        ...
                        /* Check sub-function addressing mode */
                        else if((gMsgContextType.msgAddInfo.reqType == UDS_FUNCTIONAL_ON_CAN_RX) 
        ...
        { 
            ...
            { 
                /*sub = 02,sub not configuration to support */
                DsdInternal_SetNegResponse(&gMsgContextType,DCM_E_SUBFUNCTIONNOTSUPPORTED);
            }
            else
            { 
        ...
        { 
            ...
            { 
                /*  sub = 03,sub not configuration to support  */
                DsdInternal_SetNegResponse(&gMsgContextType,DCM_E_SUBFUNCTIONNOTSUPPORTED);
            }
            else
            { 
...
*                
* Description:   Implementation of UDS service 0x3D
*                             
* Inputs:        None
*                
* Outputs:       None
...
*                
* Description:   Implementation of UDS service 0x3E
*                             
* Inputs:        None
*                
* Outputs:       None
...
*                
* Description:   Implementation of UDS service 0x85
*                             
* Inputs:        None
*                
* Outputs:       None
...
*                
* Description:   Implementation of UDS service 0x86
*                             
* Inputs:        None
*                
* Outputs:       None
...
*                
* Description:   Processing the response result of each UDS service.Transmit
*                negative or positive response(if not suppressed)
*                             
...
)
{
    ...
    {
        /* Suppress positive response or forbidden 0x11 and 0x12 NRC under the
...
*                
* Outputs:       result: The service Id's position in gCurrentSidMap
* 
* Limitations:   None
********************************************************************************
END_FUNCTION_HDR*/
        ...
        else
        {
           ...
           /*service = gServiceTable_1[sid];*/
            ...
            if(sid < DCM_NUM_OF_SERVICE)
            {
                ...
                {
                    /* Check service security access condition */
                      ...
                      else
                      {
                              ...
                              if((service.ServiceTable.DcmDsdSidTabSubfuncAvail == 1u))
                              {
                                  ...
                                  else
                                  {
                                      ...
                                      /* Check sub-function session condition */
                                      else if((sessiontype&gSesCtrlType) == DCM_SESSION_TYPE_NOT_CORRECT) 
                                      {
                                          if(gMsgContextType.msgAddInfo.reqType == UDS_FUNCTIONAL_ON_CAN_RX)
                                          {
                                      ...
                                      /* Check sub-function security condition */
    ...
    for(i = (uint32)0u; i < gMsgContextType.reqDataLen; i += (uint32)DCM_REQUIRE_DATA_LENGTH_2_BYTE)
    {
        ...
        for(j = 0u; j < NUMBER_OF_READ_DID; j++)
        {
            if(tempdid == gDcmDsdSubService_22[j].DcmDspDidIdentifier)
            {
                ...
                /* check if other condition are fulfilled */
...
)
{
    ...
    if(gSecurityDelayFlag == DCM_FLAG_DISACTIVE)
    {
        ...
        for(i = 0u; i < KIND_OF_SECURITY_LEVEL; i++)
        {
            if(gSecurityAcessDelayTime[i] == (gDcmDspSecurityRow[i].DcmDspSecurityDelayTime/DCM_TASK_TIME))
            {
                gSecurityAcessAttempNumber[i] = gDcmDspSecurityRow[i].DcmDspSecurityNumAttDelay;
            }
        }
...
//    uint8 i;
    ...
    {   
        /*If key is not valid,decrease allowed attempt number */
        DsdInternal_AccessAttemptNumberDec();         
    }
    
    /* If the key is valid,unlock related security level and reset related variable */
    else
    {   
...
*                
* Description:   Set routine started sequence flag
*                             
* Inputs:        None
*                
* Outputs:       None
...
*                
* Description:   Set routine stopped sequence flag
*                             
* Inputs:        None
*                
* Outputs:       None
...
*                
* Description:   Initial common global parameters
*                             
* Inputs:        None
*                
* Outputs:       None
...
FUNC(void,DCM_PUBLIC_CODE) Dsd_InteralInit(void)
{
    ...
    gSubFunctionHandler = DCM_NULL;
    gMsgContextType.resDataLen = (uint32)0u;
    gMsgContextBackup.resDataLen = (uint32)0u;
    Reset_Max_Number_Of_RCRRP() ;
}
