...
#define DEM_QUEUE_CYCLE_STOPPED                  DEM_CYCLE_STATE_END  /*(1U)*/
#define DEM_QUEUE_CYCLE_STARTED                  DEM_CYCLE_STATE_START  /*(2U)*/
...
//STATIC void Dem_StoreEventData(Dem_EventIdType EventId);                      //Remove by Wood, in order to resolve unused warning
//STATIC void Dem_StoreFreezeFrame(Dem_EventIdType EventId,uint8 Entry);        //Remove by Wood, in order to resolve unused warning
//STATIC void Dem_SetIndicatiorStatus(Dem_EventIdType EventId);                 //Remove by Wood, in order to resolve unused warning
STATIC Std_ReturnType Dem_DataCopy(uint8* Dest,const uint8* Src,uint16 Cnt);
STATIC boolean  Dem_EnableDtcSetting=FALSE;
STATIC Std_ReturnType Dem_GetDTCStatusAvailabilityMask(uint8* DTCStatusMask);


/* Internal variable definitions.*/
/*STATIC*/ Dem_StorageMemType DemStorageMem;
...
//STATIC uint16 Dem_EventFFMemOffset[DEM_NUMBER_OF_EVENT][DEM_CFG_MAXNUMBEROFRECORDNUMBER];     //Remove by Wood, in order to resolve unused warning
...
//STATIC uint8 Dem_SaveEvStReqFlag[(uint8)(DEM_NUMBER_OF_EVENT/8)+1]={0};                       //Remove by Wood, in order to resolve unused warning
STATIC uint8 Dem_EventCaptureFlag[(uint8)(DEM_NUMBER_OF_EVENT/8)+1]={0};
//STATIC uint16   EventInfoId = 0;                                                              //Remove by Wood, in order to resolve unused warning
//STATIC uint16   EventCaptureId = 0;                                                           //Remove by Wood, in order to resolve unused warning
//STATIC uint16   ClrDTCCounter = 0;                                                            //Remove by Wood, in order to resolve unused warning
...
)
{
    switch(det)
    {
        ...
        case DemExternalCS:
            ...
            {
                (void)(*Dem_ExternalCSDataElementClassTab[dei].DemDataElementReadFnc)(Dem_ExternalCSDataElementClassTab[dei].DemDataElementDataSize,*ppBuff);
...
STATIC boolean Dem_CaptureFreezeFrame(Dem_EventIdType EventId)
{
    uint16 NumRecNum;/**/
    uint16 NumOfDID;/**/
    uint16 i,j/*,k*/;   //Remove variable k by Wood, in order to resolve unused warning
    Dem_EventMemoryEntryType* _pMemEntry;
    uint8* _pBuff;
//    uint16 did;       //Remove by Wood, in order to resolve unused warning
...
//STATIC Std_ReturnType L_DemGetEventRecNumIndex    //Remove by Wood, in order to resolve unused warning
//(
//    Dem_EventIdType EventId,
//    uint8 RecordNumber,
//    uint8* Index/*out*/
...
{
//    uint8 loop;   //Remove by Wood, in order to resolve unused warning
    boolean result = TRUE;
...
STATIC void Dem_CycleStopProcess(Dem_OperationCycleIdType OperationCycleId)
{
    ...
    for(EvnIdx = 0; EvnIdx < DEM_NUMBER_OF_EVENT; EvnIdx++)
    {
        ...
        {
//            uint8 ucCurrentStat=0;    //Remove by Wood, in order to resolve unused warning
    ...
    for(EvnIdx = 0; EvnIdx < DEM_NUMBER_OF_EVENT; EvnIdx++)
    {
        if(OperationCycleId == DemEventClassSet[EvnIdx].DemAgingCycleRef)
        {
            ...
            */
            ...
            {
                Dem_DtcStatusInfo[DemEventParameterTab[EvnIdx].DemDTCClassRef].DtcStatus = lNewEventStatus & dtcStatusAvailabilityMask;
            }
...
//    Dem_EventIdType eventId;      //Remove by Wood, in order to resolve unused warning
//    Std_ReturnType ret;           //Remove by Wood, in order to resolve unused warning
    uint8 datsize=0;
...
void Dem_Shutdown(void)
{
    ...
    uint16 ldemstate;
//    Std_ReturnType ucRet = E_OK;  //Remove by Wood, in order to resolve unused warning
...
{
//    boolean testFailed;                       //Remove by Wood, in order to resolve unused warning
//    Dem_EventStatusExtendedType oldStatus;    //Remove by Wood, in order to resolve unused warning
    uint8 lOldEventStatus;
    uint8 lNewEventStatus;

//    uint8 ChangeFlagIndex=0;                  //Remove by Wood, in order to resolve unused warning
//    uint8 DebounceAlgorithm=0;                //Remove by Wood, in order to resolve unused warning
    uint8 dtcStatusAvailabilityMask;
    ...
    if((Dem_GetCurrentCycleStatus(Dem_GetEventOperationCycleId(EventId)) == DEM_QUEUE_CYCLE_STARTED)&&(EventId < DEM_NUMBER_OF_EVENT))
    {
        ...
        if(TRUE == (*(pDebounceInfo.Common.Calculate))(EventId,&pDebounceInfo))
        {
                ...
                if((DEM_TEST_UDS_STATUS_TF(lNewEventStatus))&&(!DEM_TEST_UDS_STATUS_TF(lOldEventStatus)))
                {
                    ...
                    if(DemStorageMem.EventExtendStatus[EventId].ucFaultPendingCounter < DemEventClassSet[EventId].DemEventFailureCycleCounterThreshold)
                    {
                        ...
                        if(DemStorageMem.EventExtendStatus[EventId].ucFaultPendingCounter == DemEventClassSet[EventId].DemEventFailureCycleCounterThreshold)
                        {
                            ...
                            /*Record DTC to EEPROM*/
                            NvM_WriteBlock(DEM_NVM_BLOCKID,(uint8*)(&DemStorageMem));
                        }
                    }
                }
            }
            ...
            Dem_DtcStatusInfo[DemEventParameterTab[EventId].DemDTCClassRef].DtcStatus = lNewEventStatus & dtcStatusAvailabilityMask;
...
{
    Std_ReturnType ret = E_NOT_OK;
//    Std_ReturnType lReturnValue;  //Remove by Wood, in order to resolve unused warning
    uint8 lOldEventStatus;
    uint8 lNewEventStatus;
...
//    Dem_EventIdType i;        //Remove by Wood, in order to resolve unused warning
//    uint8 didIndex;           //Remove by Wood, in order to resolve unused warning
//    uint8 didInfo;            //Remove by Wood, in order to resolve unused warning
//    uint8 didDataSize=0;      //Remove by Wood, in order to resolve unused warning
...
{
//    uint8 byteidx;                    //Remove by Wood, in order to resolve unused warning
//    uint8 bitidx;                     //Remove by Wood, in order to resolve unused warning
//    uint8 bytenum;                    //Remove by Wood, in order to resolve unused warning
//    uint16 eventId;                   //Remove by Wood, in order to resolve unused warning
    uint16 ldemstate;
//    Std_ReturnType ucRet = E_OK;      //Remove by Wood, in order to resolve unused warning
...
{
//    Dem_EventIdType i;            //Remove by Wood, in order to resolve unused warning
//    Dem_EventIdType dtcNumber=0;  //Remove by Wood, in order to resolve unused warning
...
{
//	uint8 i;    //Remove by Wood, in order to resolve unused warning
    uint16 dtcidx;
...
//    for(i=Dem_DTCFilterInfo.NextFilteredDTCIndex;i<DEM_NUMBER_OF_EVENT;i++)
//    {
...
{
//    Dem_EventIdType i;    //Remove by Wood, in order to resolve unused warning
//    uint8 recOcc;         //Remove by Wood, in order to resolve unused warning
    uint8* pffdata;
    uint16 dtcidx;
    uint16 eveidx;
    uint16 len;
...
//    uint8 recOcc;             //Remove by Wood, in order to resolve unused warning
//    uint8 recNum=0;           //Remove by Wood, in order to resolve unused warning
//    uint8 dataIndex=0;        //Remove by Wood, in order to resolve unused warning
//    uint8 ffDataSize=0;       //Remove by Wood, in order to resolve unused warning
    uint8* pffdata;
    uint16 dtcidx;
    uint16 eveidx;
    uint16 len;
...
{
//    uint8 result=0;   //Remove by Wood, in order to resolve unused warning
    uint8 dtcStatusAvailabilityMask;
...
*
******************************************************************************/
void J1939_DM1_monitor(void)
{
	PduInfoType sdu_dm1;
	static uint8 tp_dt_flag;
	static uint8 data[100];
	static uint8 pack_num;
	static uint8 dtc_len;
	static uint8 i=0,j=0;
	static uint8 timer_counter;
	uint8 dm1_data[8];
    uint32 dtc;
	   uint8 dtcStatus;
	   uint8 dtcStatusMask;
	...
	sdu_dm1.SduLength = CAN_DATA_DLC;
	sdu_dm1.SduDataPtr = (uint8*)dm1_data;
	...
	if(timer_counter >= 100)
	{
       ...
       else
       {
           do
           {
               ...
               if(getNextDtcResult == DEM_FILTERED_OK)
               {
					data[0+dtcCounter*4] = dtc>>16;
					data[1+dtcCounter*4] = dtc>>8;
					data[2+dtcCounter*4] = dtc;
					data[3+dtcCounter*4] = dtcStatus;
                   dtcCounter++;
               }
           }
           while(getNextDtcResult != DEM_FILTERED_NO_MATCHING_DTC);
		...
		if(1 == dtcCounter)
    {
			/*DM1 Message transmits*/
			dm1_data[0] = dtcCounter;
			dm1_data[1] = 0xFF; //Reserved byte
			dm1_data[2] = data[0];
			dm1_data[3] = data[1];
			dm1_data[4] = data[2];
			dm1_data[5] = data[3];
			dm1_data[6] = 0xFF; //Unused byte
			dm1_data[7] = 0xFF; //Unused byte
			CanIf_Transmit(TP_DM1_TXPDUID,&sdu_dm1);
		}
		else if(1 < dtcCounter)
		{
			tp_dt_flag = TRUE;
			dtc_len = dtcCounter*4;
			pack_num = (dtc_len/7)+1;
			/*TP.CM_BAM transmits*/
			dm1_data[0] = 0x20; //Stand for TP.CM_BAM
			dm1_data[1] = 0x00; //DTC number is 25. 25*4 =100 byte < 255
			dm1_data[2] = dtcCounter*4;
			dm1_data[3] = pack_num;
			dm1_data[4] = 0xFF; //Reserved byte
			dm1_data[5] = 0xCA; //FECAE0 => DM1 SPN and Address
			dm1_data[6] = 0xFE;
			dm1_data[7] = 0xE0;
			CanIf_Transmit(TP_BAM_TXPDUID,&sdu_dm1);
		}
		else
		{}
	}
	else if(tp_dt_flag)
	{
		if(i < pack_num)
		{
			dm1_data[0] = i+1; //Sequence
			for(j = 0 ; j < 7 ; j++)
			{
				if((dtc_len-j-1)>=0)
				{
					dm1_data[j+1] = data[i*7+j];
				}
				else
		{
					dm1_data[j+1] = 0xFF;
			...
			CanIf_Transmit(TP_DT_TXPDUID,&sdu_dm1);
			i++;
		}
		else
		{
...
/******************************************************************************
* Name         :Dem_SetEventStatus_By_Parameter
* Called by    :Dcm
* Preconditions:N/A
* Parameters   :
* Return code  :
...
*
******************************************************************************/
void Dem_SetEventStatus_By_Parameter(uint16 val)
{
    uint16 eventidtype = 0;
    uint8 eventstatustype = 0;

    eventidtype = (uint16) ((val>>8)&0xFF);
    eventstatustype = (uint8) (val&0xFF); 

    Dem_SetEventStatus(eventidtype, eventstatustype);
}
