...
/* L100: */
		...
		for (l = 1; l <= i__2; ++l) {
			...
			R_CNJG(&q__2, &b[j + l * b_dim1]);  //r_cnjg(&q__2, &b[j + l * b_dim1]);
			...
			R_CNJG(&q__1, &q__2);   //r_cnjg(&q__1, &q__2);
...
/* L190: */
			...
			if (*beta == 0.f) {
			    ...
			    R_CNJG(&q__4, alpha);   //r_cnjg(&q__4, alpha);
			...
			if (*beta == 0.f) {
			    ...
			    R_CNJG(&q__6, alpha);   //r_cnjg(&q__6, alpha);
