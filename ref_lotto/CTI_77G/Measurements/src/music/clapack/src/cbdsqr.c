...
extern int slas2_(real *, real *, real *, real *, real *);
extern int cswap_(integer *, complex *, integer *, complex *, integer *);
extern int csrot_(integer *, complex *, integer *, complex *, integer *, real *, real *);
extern int slasq1_(integer *, real *, real *, real *, integer *); 
extern int slasv2_(real *, real *, real *, real *, real *, real *, real *, real *, real *);
...
/*        Update singular vectors if desired */
	...
	if (*nru > 0) {
	    clasr_RVF(*nru, *n, &rwork[1], &rwork[*n], &u[u_offset], *ldu);
	}
	if (*ncc > 0) {
	    clasr_LVF(*n, *ncc, &rwork[1], &rwork[*n], &c__[c_offset], *ldc);
	}
    }
...
/*        First apply standard test to bottom of matrix */
    ...
    r__1 = d__[m], dabs(r__1);
    r__2 = e[m - 1], dabs(r__2);
    r__3 = e[m - 1], dabs(r__3);
	if (r__2 <= dabs(tol) * r__1 || (tol < 0.f && r__3 <= thresh)) 
	{
	    e[m - 1] = 0.f;
	    goto L60;
	}
...
/*           Save cosines and sines for later singular vector updates */
		...
		r__1 = d__[i__] * cs;
		slartg_x(r__1, e[i__], &cs, &sn, &r__);  // change from slartg_
		if (i__ > ll) {
		    e[i__ - 1] = oldsn * r__;
		}
		r__1 = oldcs * r__;
		r__2 = d__[i__ + 1] * sn;
		slartg_x(r__1, r__2, &oldcs, &oldsn, &d__[i__]);    // change from slartg_
		rwork[i__ - ll + 1] = cs;
...
/*           Update singular vectors */
	    ...
	    if (*ncvt > 0) {
		i__1 = m - ll + 1;
		clasr_LVF(i__1, *ncvt, &rwork[1], &rwork[*n], &vt[ll + vt_dim1], *ldvt);
	    }
	    if (*nru > 0) {
		i__1 = m - ll + 1;
		clasr_RVF(*nru, i__1, &rwork[nm12 + 1], &rwork[nm13 + 1], &u[ll * u_dim1 + 1], *ldu);
	    }
	    if (*ncc > 0) {
		i__1 = m - ll + 1;
		clasr_LVF(i__1, *ncc, &rwork[nm12 + 1], &rwork[nm13 + 1], &c__[ll + c_dim1], *ldc);
	    }
...
/*           Save cosines and sines for later singular vector updates */
		...
		r__1 = d__[i__] * cs;
		slartg_x(r__1, e[i__ - 1], &cs, &sn, &r__);    // change from slartg_
		if (i__ < m) {
		    e[i__] = oldsn * r__;
		}
		r__1 = oldcs * r__;
...
/*           Update singular vectors */
	    ...
	    if (*ncvt > 0) {
		i__1 = m - ll + 1;
		clasr_LVB(i__1, *ncvt, &rwork[nm12 + 1], &rwork[nm13 + 1], &vt[ll + vt_dim1], *ldvt);
	    }
	    if (*nru > 0) {
		i__1 = m - ll + 1;
		clasr_RVB(*nru, i__1, &rwork[1], &rwork[*n], &u[ll * u_dim1 + 1], *ldu);
	    }
	    if (*ncc > 0) {
		i__1 = m - ll + 1;
		clasr_LVB(i__1, *ncc, &rwork[1], &rwork[*n], &c__[ll + c_dim1], *ldc);
	    }
...
/*           Save cosines and sines for later singular vector updates */
	    ...
	    i__1 = m - 1;
	    for (i__ = ll; i__ <= i__1; ++i__) {
		slartg_x(f, g, &cosr, &sinr, &r__);    // change from slartg_
		if (i__ > ll) {
		    e[i__ - 1] = r__;
		}
		...
		slartg_x(f, g, &cosl, &sinl, &r__);    // change from slartg_
		d__[i__] = r__;
